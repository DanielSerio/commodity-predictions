...
import { getUnreviewedPredictionsCount } from '@/repositories/predictions';
...
export async function DashboardPage() {
  ...
  const isOnline = await getOllamaStatus();

  const pendingReviewsResult = getUnreviewedPredictionsCount();
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          ...
          >
            <Card
              className={cn(
                'hover:bg-primary/5 transition-all duration-300 cursor-pointer group hover:scale-[1.02]',
